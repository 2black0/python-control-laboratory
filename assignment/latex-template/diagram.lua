...
local function code_to_figure (conf)
  return function (block)
    ...
    local supported_diagrams = {
      plantuml = true,
      mermaid = true,
      dot = true,
      tikz = true,
      cetz = true,
      asymptote = true, -- Tambahkan engine lain yang Anda gunakan di sini!
    }

    if not diagram_type or not supported_diagrams[diagram_type] then
      return nil -- Lewati blok kode ini jika bukan diagram yang didukung.
    end

    local engine = conf.engine[diagram_type]
    if not engine then
      return nil  -- Ini seharusnya tidak terjadi, tapi tetap ditangani.
    end
